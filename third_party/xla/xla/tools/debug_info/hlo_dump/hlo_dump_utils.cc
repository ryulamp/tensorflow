...
absl::flat_hash_map<TensorKey, TensorAnnotation> PopulateMismatchAnnotations(
  ...
  absl::flat_hash_map<std::string, const HloInstruction*> instrs_by_name;
  for (const HloComputation* comp : module.computations()) {
    for (const HloInstruction* instr : comp->instructions()) {
      instrs_by_name[std::string(instr->name())] = instr;
    }
  }

  for (const auto& mismatch : mismatches) {
    auto it = instrs_by_name.find(mismatch.target_instruction_name);
    if (it == instrs_by_name.end()) {
      continue;
    }
